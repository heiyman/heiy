...
// Everything now lives flat in the repo root (no /public folder) since that's
// how files land when uploaded one-by-one from a phone. To avoid exposing
// server.js, store.js, package.json, etc. over the web, only these specific
// front-end files are served — nothing else in the folder is reachable by URL.
const PUBLIC_FILES = [
  'index.html', 'admin.html', 'style.css',
  'app.js', 'admin.js', 'outbox.js',
  'manifest.json', 'sw.js',
];
